...
export default async function FornecedorPage({
  ...
  const [
    ...
    contratoIds.length
      ? supabase
          ...
          .select(
            "id,contrato_id,data_evento,tipo,titulo,resumo,natureza,valor_cents,despesa_id,movimento_id,efeito,contrato_memoria_evidencias(id,localizador,citacao,papel,ia_documental_fontes(id,titulo,referencia,url))",
          )
          .eq("tenant_id", tenantId)
  ...
  const eventosVisiveis =
    grupoAtivo === "tudo" ? eventos : eventos.filter((evento) => evento.group === grupoAtivo);
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Resumo label="Despesas registadas" valor={formatCurrency(resumo.despesasRegistadasCents)} />
        <Resumo label="Saídas confirmadas" valor={formatCurrency(resumo.saidasConfirmadasCents)} nota="Débitos com prova bancária" />
        <Resumo
          label="Em aberto"
          valor={formatCurrency(resumo.emAbertoCents)}
          destaque={resumo.emAbertoCents > 0}
          nota={resumo.condicionadoCents > 0 ? `dos quais ${formatCurrency(resumo.condicionadoCents)} retidos` : undefined}
        />
        <Resumo
                ...
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {GRUPOS_TIMELINE.map((grupo) => {
                    ...
                    return (
                      <Link
                        key={grupo.valor}
                        href={grupo.valor === "tudo" ? `/fornecedores/${id}` : `/fornecedores/${id}?vista=${grupo.valor}`}
        ...
        <aside className="space-y-6">
          ...
          <section className="portaria-panel overflow-hidden">
            <div className="border-b border-britishGreen/10 px-5 py-4">
              ...
              <p className="mt-1 font-body text-[0.7rem] leading-4 text-oliveGray">
                Fontes do dossiê. Um documento é fonte; um acontecimento vive no histórico.
              </p>
            </div>
            {docs.length > 0 && (
...
function Evento({ evento }: { evento: SupplierTimelineEvent }) {
        ...
        {evento.evidenceCount > 0 && (
          <details className="mt-2">
            <summary className="cursor-pointer list-none font-body text-[0.68rem] font-semibold text-britishGreen">
              Ver evidência ({evento.evidenceCount})
            </summary>
            <ul className="mt-2 space-y-2 border-l-2 border-britishGreen/15 pl-3">
              {evento.evidence.map((evidencia) => (
                <li key={evidencia.id} className="font-body text-[0.7rem] leading-5 text-oliveGray">
                  <span className="font-semibold text-ink">{evidencia.fonte?.titulo ?? "Fonte documental"}</span>
                  {evidencia.localizador ? ` · ${evidencia.localizador}` : ""}
                  {evidencia.papel !== "primaria" ? ` · ${evidencia.papel}` : ""}
                  <br />
