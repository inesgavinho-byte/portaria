...
export default async function RelatorioFornecedorPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ ano?: string; modo?: string }> }) {
  ...
  const [{ data: despesas }, { data: memoria }] = await Promise.all([despesaQuery.order("data_documento", { ascending: true }), memoriaQuery]);
  ...
  return <article className="mx-auto max-w-6xl pb-12 print-report">
    ...
    <Secao titulo="Resumo executivo"><div className="grid gap-px border border-britishGreen/15 bg-britishGreen/15 sm:grid-cols-2 lg:grid-cols-4"><Metrica label="Contratos" valor={String(cts.length)} /><Metrica label="Valor adjudicado conhecido" valor={cts.some((c) => c.valor !== null) ? euro(cts.reduce((s, c) => s + (c.valor ?? 0) * 100, 0)) : "Não documentado"} /><Metrica label="Total facturado" valor={euro(totalFacturado)} /><Metrica label="Confirmado no banco" valor={euro(confirmadoBanco)} /><Metrica label="Pago documentalmente" valor={euro(pagoDocumental)} /><Metrica label="Saldo documental" valor={euro(saldo)} /><Metrica label="Valor retido" valor={retido ? euro(retido) : "—"} /><Metrica label="Em conflito" valor={conflitos.length ? `${conflitos.length} ponto(s)` : "—"} /></div></Secao>
