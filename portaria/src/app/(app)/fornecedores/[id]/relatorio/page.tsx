...
const data = (value: string | null) => value ? new Intl.DateTimeFormat("pt-PT", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(value)) : "—";
const anoDe = (value: string | null) => value ? String(new Date(value).getFullYear()) : null;
...
export default async function RelatorioFornecedorPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ ano?: string; modo?: string }> }) {
  ...
  const [{ data: despesas }, { data: memoria }, { data: fontesDocumentais }] = await Promise.all([
    ...
    memoriaQuery,
    supabase.from("ia_documental_fontes").select("id,titulo,referencia,url,conteudo_markdown").eq("tenant_id", ctx.tenant.id).eq("ativa", true),
  ]);
  ...
  const valoresDocumentais = resolverValoresDocumentais((fontesDocumentais ?? []) as FonteDocumentalValor[]);
  const valorLateral = valoresDocumentais.lateral[0];
  ...
  return <article className="mx-auto max-w-6xl pb-12 print-report">
    <div className="no-print mb-6 flex items-center justify-between gap-3"><Link href={`/fornecedores/${id}`} className="inline-flex items-center gap-1 font-body text-xs font-semibold uppercase tracking-widest text-oliveGray hover:text-britishGreen"><ChevronLeft className="h-3.5 w-3.5" /> Fornecedor</Link><RelatorioFornecedorImprimir /></div>
    ...
    {!financeiro && <Secao titulo="Fontes e evidências">{fontes.length ? <div className="divide-y divide-britishGreen/10 border-y border-britishGreen/15">{fontes.map((fonte) => <div key={fonte.id} className="py-3"><p className="font-body text-sm font-medium text-ink">{fonte.titulo}</p>{fonte.referencia && <p className="mt-1 font-body text-xs text-oliveGray">{fonte.referencia}</p>}{fonte.url && <a href={fonte.url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 font-body text-xs text-britishGreen"><ExternalLink className="h-3 w-3" /> Ver fonte</a>}</div>)}</div> : <Vazio>Não existem fontes documentais ligadas aos eventos deste período.</Vazio>}</Secao>}
    <footer className="mt-12 border-t border-britishGreen/20 pt-5 font-body text-xs leading-5 text-oliveGray">Documento gerado a partir do registo operacional e documental do PORTARIA.<br />Factos, inferências, conflitos e pendências são apresentados segundo a classificação existente no sistema à data de geração.</footer>
  </article>;
}

function fontesUnicas(eventos: ContratoMemoriaEvento[]) { const mapa = new Map<string, Fonte>(); eventos.forEach((e) => e.contrato_memoria_evidencias.forEach((ev) => ev.ia_documental_fontes.forEach((fonte) => mapa.set(fonte.id, fonte)))); return [...mapa.values()]; }
...
function ListaEventos({ eventos }: { eventos: ContratoMemoriaEvento[] }) { return <div className="divide-y divide-britishGreen/10 border-y border-britishGreen/15">{eventos.map((e) => <div key={e.id} className="py-4"><p className="font-body text-xs uppercase tracking-widest text-oliveGray">{data(e.data_evento)}</p><p className="mt-1 font-body text-sm font-semibold text-ink">{e.titulo}</p><p className="mt-1 font-body text-sm leading-6 text-oliveGray">{e.resumo}</p></div>)}</div>; }
